...
VERSION="1.0.2"
#Version history
#1.0.0          Alex Lam                first version.
#1.0.1          Alex Lam                added warning output to INFO_FILE.
#                                       changed some ERRORS to WARNINGS
#1.0.2          Alex Lam                added support for ciass device files.
...
# Infofile
INFO_FILE="/etc/epmf/systemcheck.info"

# systemcheck csv file
SYSTEMCHECK_CSV=/tmp/${HOSTNAME}-systemcheck.csv
...
func_msg()
{
        ...
        MSG_TERMWITH=$(tput -Tvt100 cols); RC=$?
        if [[ "${RC}" != "0" ]]
        then
                MSG_TERMWITH="80"
        fi
...
if [[ ${APPCOM} = "1" ]]; then
        ...
        echo ${CSV_OUTPUT} > ${SYSTEMCHECK_CSV}; RC=$?
        if [[ ${RC} = "0" ]]; then
                func_msg INFO "Created file ${SYSTEMCHECK_CSV}"
        else
                func_msg INFO "Error while creating file ${SYSTEMCHECK_CSV}"
        fi
fi
}
...
Copyright (C) $(date +"%Y") by Atos Information Technology HK Ltd 
...
echo " ";
#echo -n "CSV;";
#cat "$SYSTEMCHECK_CSV" |sed "s/-Loc/;-Loc/g"|awk -F';' '{print $1";"$4";"$6";"$8";"$10";"$12";"$14";"$16}'
